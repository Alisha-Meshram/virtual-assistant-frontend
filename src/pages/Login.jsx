...
const Login = () => {
   ...
   async function login(e){
    ...
    setLoading(true)
    try {
      const res=await axios.post(`${serverUrl}/api/v1/login`,{email,password},{withCredentials:true})
  ...
  return (
    ...
    >
      <form onSubmit={login} className="w-[90%] h-[500px] max-w-[500px] flex flex-col justify-center items-center shadow-lg shadow-black backdrop-blur bg-[#00000062] gap-[20px] mb-[20px] px-[20px]">
        ...
        <button type="submit" className="min-w-[150px] h-[60px] text-black bg-white rounded-full text-[19px] font-semibold" disabled={loading}>{loading?"loading..":"Sign In"}</button>
        <p className="text-[white] text-[18px] cursor-pointer"> Want to create new account ? <span className="text-blue-400" onClick={()=>{navigate('/register')}}>Sign Up</span></p>
      </form>
    </div>
  );
};
